...
export function draw(ctx, canvas, dt) {
    ...
    drawParallaxLayer(ctx, canvas, camera, parallaxLayers.clouds, (el) => { ctx.fillStyle = `rgba(255, 255, 255, ${el.alpha})`; ctx.beginPath(); ctx.ellipse(el.x, el.y, el.radiusX, el.radiusY, 0, 0, Math.PI * 2); ctx.fill(); });
    const drawMountain = (el) => { ctx.beginPath(); ctx.moveTo(el.x + el.points[0].x, el.y + el.points[0].y); for(let i = 1; i < el.points.length; i++) { ctx.lineTo(el.x + el.points[i].x, el.y + el.points[i].y); } ctx.closePath(); ctx.fill(); };
